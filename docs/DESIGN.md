...
нет), устойчивое хранилище объяснений (`MemStore` держит их in-process и растёт
без вытеснения - за интерфейсом `Store`, адаптер с TTL или БД позже), развёрнутый
сетевой сервис, нагрузочный тест против SLA, эффективность детекции (модель
скромная и не тюнингованная - по дизайну доказывается путь, не качество).
