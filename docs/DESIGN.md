...
- **точная атрибуция** - вклады SHAP из того же предиктора, что скорит вход;
...
IP, страна и ASN - тоже, и их глобальная частота читает метку почти напрямую - 98.5%
...
> Точный нативный код причины на момент инференса возвращает только тот C-ABI, что сам отдаёт вклады.
...
LightGBM и XGBoost отдают вклады на инференсе; CatBoost-applier и ONNX - нет. Чистая
реализация на Go проигрывает *дважды*: считает только предсказание, без вкладов, и
