...
цель Is Attack IP), закоммиченная, чтобы примеры `http/` и `postman/` работали на
...
Затем гонять `http/scorer.http` (JetBrains HTTP Client) или `postman/` (порт по
умолчанию :8080).
...
`python/train.py` из списка `RBA_FEATURES`.
